...
The scan is driven by [`policy/prompt.md`](policy/prompt.md) — a single
markdown file that defines what "safe" means. Edit that file to change the
policy everywhere. To use a repo-local prompt instead, pass `policy-prompt:`
pointing at a file in the consuming repo.

The expected verdict shape is in [`policy/schema.json`](policy/schema.json).
